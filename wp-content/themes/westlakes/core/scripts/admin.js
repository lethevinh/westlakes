...
jQuery( function( $ ) {
    ...
    $( '#upsell_meta_data' ).bind( 'keypress', function( e ) {
        if ( e.keyCode === 13 ) {
            return false;
        }
    });
    ...
    function show_and_hide_panels() {
        var meta_type    = $( 'select#product-type' ).val();
        ...
        $.each( woocommerce_admin_meta_boxes.meta_types, function( index, value ) {
        ...
        $( '.woocommerce_options_panel' ).each( function() {
            ...
            if ( $invisble.length === $children.length ) {
                var $id = $( this ).prop( 'id' );
                $( '.meta_data_tabs' ).find( 'li a[href="#' + $id + '"]' ).parent().hide();
            }
        });
    }
    ...
    $( 'input#_manage_stock' ).change( function() {
        if ( $( this ).is( ':checked' ) ) {
            ...
            $( 'p.stock_status_field:not( .hide_if_' + meta_type + ' )' ).show();
        }
    ...
    // Initial order.
    var woocommerce_attribute_items = $( '.meta_attributes' ).find( '.woocommerce_attribute' ).get();
    ...
    $( woocommerce_attribute_items ).each( function( index, el ) {
        $( '.meta_attributes' ).append( el );
    });

    function attribute_row_indexes() {
        $( '.meta_attributes .woocommerce_attribute' ).each( function( index, el ) {
            $( '.attribute_position', el ).val( parseInt( $( el ).index( '.meta_attributes .woocommerce_attribute' ), 10 ) );
        });
    }
    ...
    $( 'button.add_attribute' ).on( 'click', function() {
        var size         = $( '.meta_attributes .woocommerce_attribute' ).length;
        var attribute    = $( 'select.attribute_taxonomy' ).val();
        var $wrapper     = $( this ).closest( '#meta_attributes' );
        var $attributes  = $wrapper.find( '.meta_attributes' );
        var meta_type = $( 'select#product-type' ).val();
        var data         = {
    ...
    $( '.meta_attributes' ).on( 'blur', 'input.attribute_name', function() {
        $( this ).closest( '.woocommerce_attribute' ).find( 'strong.attribute_name' ).text( $( this ).val() );
    });

    $( '.meta_attributes' ).on( 'click', 'button.select_all_attributes', function() {
        $( this ).closest( 'td' ).find( 'select option' ).attr( 'selected', 'selected' );
        $( this ).closest( 'td' ).find( 'select' ).change();
        return false;
    });

    $( '.meta_attributes' ).on( 'click', 'button.select_no_attributes', function() {
        $( this ).closest( 'td' ).find( 'select option' ).removeAttr( 'selected' );
        $( this ).closest( 'td' ).find( 'select' ).change();
        return false;
    });

    $( '.meta_attributes' ).on( 'click', '.remove_row', function() {
    ...
    $( '.meta_attributes' ).on( 'click', 'button.add_new_attribute', function() {
    ...
    $( '.save_attributes' ).on( 'click', function() {
        ...
        var original_data = $( '.meta_attributes' ).find( 'input, select, textarea' );
        var data = {
            post_id     : woocommerce_admin_meta_boxes.post_id,
            meta_type: $( '#product-type' ).val(),
        ...
        $.post( woocommerce_admin_meta_boxes.ajax_url, data, function( response ) {
            if ( response.error ) {
                ...
                // Success.
                $( '.meta_attributes' ).html( response.data.html );
                $( '.meta_attributes' ).unblock();
                ...
                $( '#variable_meta_options' ).load( this_page + ' #variable_meta_options_inner', function() {
                    $( '#variable_meta_options' ).trigger( 'reload' );
                } );
            }
        });
    });
    ...
    var $image_gallery_ids = $( '.meta_gallery_input' );
    var $meta_images    = $( '#meta_images_container' ).find( 'ul.meta_images' );

    $( '.add_meta_images' ).on( 'click', 'a', function( event ) {
        var $el = $( this );
        ...
        // Create the media frame.
        meta_gallery_frame = wp.media.frames.meta_gallery = wp.media({
            // Set the title of the modal.
            title: $el.data( 'choose' ),
            button: {
                text: $el.data( 'update' )
    ...
    $meta_images.sortable({
        ...
        update: function() {
            ...
            $( '#meta_images_container' ).find( 'ul li.image' ).css( 'cursor', 'default' ).each( function() {
    ...
    // Remove images.
    $( '#meta_images_container' ).on( 'click', 'a.delete', function() {
